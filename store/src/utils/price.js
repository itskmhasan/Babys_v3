...
export const getDiscountPercentage = (price, originalPrice) => {
  const normalized = normalizePricePair(price, originalPrice);
  if (!normalized.hasDiscount || normalized.originalPrice <= 0) return 0;

  return Math.round(
    ((normalized.originalPrice - normalized.price) / normalized.originalPrice) *
      100
  );
};
